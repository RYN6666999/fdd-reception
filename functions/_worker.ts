...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const isRelayHost = host.startsWith('aris.') || host.startsWith('aris-live.')

    if ((method === 'GET' || method === 'HEAD') && path === '/' && isRelayHost) {
      return Response.redirect(`${url.origin}/relay-ui/`, 302)
    }
    ...
    // Relay endpoints
    if (method === 'OPTIONS' && (path === '/c' || path === '/health' || path === '/admin/status' || path === '/admin/probe' || path.startsWith('/events/'))) {
    ...
    if (method === 'POST' && path === '/admin/probe') return handleRelayAdminProbe(request, env)
