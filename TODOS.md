...
## Phase 3b — 信用卡背面 OCR ✅（2026-06-10，提前觸發）

> 原觸發條件是失敗率 > 15%。真實照片實測直接證實：新式卡（卡號印在背面）
> 正面 OCR 必然失敗，這是卡片設計不是模型問題 → 立刻做。

- [x] 後端 `photo.ts` card_back prompt 改為輔助辨識卡號/到期日（合成新式卡背面實測通過）
- [x] CVV 雙重防護：prompt 明令不辨識 + server 端把 cvv/cvc/security_code 鍵剝除後才回傳
- [x] card_back 辨識結果僅在通過 Luhn / 格式檢核時回傳；沒讀到不算失敗（輔助性質）
- [x] client：背面讀到的卡號/到期日自動補進空欄位；正面失敗時顯示「新式卡卡號常印在背面」提示

## Phase 10 — 真實測試回饋修復 ✅（2026-06-10）

- [x] Token 逾時只說「送出失敗」→ 新增 TTL 倒數 banner（<2 分鐘加強提示）、
  逾時自動跳失效畫面、410/400/409 各自有明確訊息
- [x] 送出前 client 端欄位檢核（Luhn / 到期日 / 身分證檢核碼），錯誤訊息指名欄位並跳回該步驟
- [x] 卡號輸入自動四碼分組、到期日自動補 `/`（inputmode=numeric）
- [x] init() 接線順序修正：導覽/欄位/送出接線不再排在相機 await 之後
  （相機權限對話框 pending 曾導致整頁按鈕半死）；拍照鈕在相機就緒前 disabled（防黑圖）
- [x] 真實照片 OCR 驗證：傳統凸字卡正面 ✅、新式卡背面（合成）✅、
  身分證實照 ✅；新式卡正面無卡號屬卡片設計（被遮蔽樣本模型正確回 null）
